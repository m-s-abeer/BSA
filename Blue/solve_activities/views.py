...
class SolveConfirmation(LoginRequiredMixin, generic.TemplateView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context["solve_status"] = self.request.user.is_authenticated and context["prob"].solved.filter(
            solver=self.request.user).exists()
        return context
...
@login_required
def ProblemSolvedXHR(request, slug):
